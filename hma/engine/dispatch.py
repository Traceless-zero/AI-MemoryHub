...
def _cmd_derive(a):
    """对 built 记忆重派生章级锚点（扫描 `##` 标题树，整组覆盖写回）。

    注意：全库锚点已由人工手写扎实（front-matter V2 契约），derive 命令
    是【纯派生覆盖】工具——会丢弃手写 about/keywords、重扫标题树生成派生版。
    仅在需要把某包锚点退回"机械派生基线"时显式使用；常规维护不要跑它，
    否则会覆盖人工锚点。幂等：派生结果与现存一致则跳过写。
    ...
    try:
        ...
        for rid, _t, _g, _u in mem.list_all():
            ...
            if not pkg:
                continue
            anchors = derive_anchors(pkg.body)          # 纯派生覆盖
        ...
        mem.close()
    print(f"派生锚点完成：更新 {n_write} 个包（root={label}，纯派生覆盖）")
