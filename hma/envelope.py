...
契约字段：
  ...
  keywords  list[str]                      必填非空；从【原句】抽的实体/上下文词，驱动硬拒答闸
                                            注：本信封强制 keywords 非空是【AI-flow 上层契约】；
                                            引擎 query_anchors 的 entity_gate 仅在 keywords is not None
                                            时启用（可整体关闭，作机械兜底逃生口）——上层强约束 + 下层
                                            宽松逃生，非矛盾：经信封的查询必带 keywords，直调引擎的机械路径
                                            则放弃硬拒答闸（符合"机械兜底不硬顶拒答"铁律）。
