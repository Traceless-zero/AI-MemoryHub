...
（澄清：本模块停用的是"等同 index.db 的全量结构派生缓存常驻"；与
`scripts/core/derive_topic_views.py` 生成的「各包开发日志.md / 主题索引.md」不冲突——
后者是带派生标记、只重写空文件/标记文件、跳过手写文件的**按需聚合视图**，非全量
结构镜像，故不算本段所警告的"污染仓库"同类物。）

原逻辑摘要：命名空间 → 包 两级树，包内 .md 不入树，隐私目录 demo-char/private 跳过。
"""
