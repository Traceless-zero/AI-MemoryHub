...
移动仓库都不用改配置。仓库根本身靠 ~/.hma_home 指针 + scripts/core/where.py
在运行时动态解析。

幂等：可重复跑；只更新 mcpServers.aimh，保留 mcp.json 里其它连接器（并移除旧 hma 键）。

用法：
  python scripts/core/deploy_mcp.py            # 部署（复制启动器 + 合并写出 mcp.json + 登记指针）
  python scripts/core/deploy_mcp.py --dry-run  # 只打印将写出的配置，不落盘
"""
import io
import os
import sys
...
REPO = os.path.dirname(os.path.abspath(__file__))   # <repo>/scripts/core
REPO = os.path.dirname(REPO)                          # <repo>/scripts
REPO = os.path.dirname(REPO)                          # <repo>
WB_HOME = os.path.join(os.path.expanduser("~"), ".workbuddy")
LAUNCHER_DST = os.path.join(WB_HOME, "aimh_mcp_launch.py")
...
def deploy(dry_run=False):
    ...
    sp = os.path.join(REPO, "scripts", "core")
    ...
    src = os.path.join(REPO, "scripts", "core", "mcp_launch.py")
    if dry_run:
        print("[dry-run] 将复制 %s -> %s" % (src, LAUNCHER_DST))
    else:
        shutil.copyfile(src, LAUNCHER_DST)

    # 2) 合并写出 mcp.json（只动 mcpServers.aimh，保留其它连接器；移除旧 hma 键）
    cfg = {"mcpServers": {"aimh": build_aimh_server()}}
    ...
    existing["mcpServers"]["aimh"] = cfg["mcpServers"]["aimh"]
    existing["mcpServers"].pop("hma", None)  # 旧前缀残留，清掉避免双注册
