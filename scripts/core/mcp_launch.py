...
被 WorkBuddy 的 mcp.json 调用，拉起 hma.server。两种部署形态都兼容：
  - 在仓库内运行（scripts/core/mcp_launch.py）：从自身 __file__ 反推仓库根。
  - 被部署到 ~/.workbuddy/ 后运行：读 where.py 登记的 ~/.hma_home 指针。
拿到仓库根后复用 scripts/core/where.py 解析 memory/，再拉起 hma.server。

即：用「定位脚本」(scripts/core/where.py) 代替写死字符串。

用法：
  python scripts/core/mcp_launch.py
调试（只打印解析出的 memory 根，不启动服务）：
  python scripts/core/mcp_launch.py --resolve-only
"""
import io
import os
import sys
...
def _self_locate_repo():
    ...
    判定用 HMA 专属标记 scripts/core/where.py，而不是只看 memory/ —— 因为
    ...
    for cand in (here, os.path.dirname(here), os.path.dirname(os.path.dirname(here))):
        if os.path.isdir(os.path.join(cand, "hma")):
            return cand
    return None
...
def resolve():
    ...
    if not repo or not os.path.isdir(os.path.join(repo, "memory")):
        raise SystemExit(
            "找不到 HMA 仓库根。请先在 HMA 仓库内运行一次： python scripts/core/where.py\n"
            "（它会把仓库根登记到 ~/.hma_home）"
        )
