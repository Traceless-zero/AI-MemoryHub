...
运行（务必在仓库根目录下，使 hma 包可被 import）：
  python scripts/core/oc_registry.py find "示例角色你在干嘛"   # 解析命中的 OC（实时扫 memory/，无需快照）
...
# 仓库根：向上找到含 hma/ 包的目录（兼容 scripts/ 与 scripts/core/ 嵌套收拢）
REPO = HERE
while REPO and not os.path.isdir(os.path.join(REPO, "hma")):
...
# ---------------------------------------------------------------------------
# 代号抽取：基础包正文里的「英雄代号：XXX」补为别名，供代号唤醒
# ---------------------------------------------------------------------------
_CODENAME_RE = re.compile(r'英雄代号\*{0,2}\s*[:：]\s*\*{0,2}\s*([\w一-鿿·]+)')


def _extract_codename(body):
    if not body:
        return None
    m = _CODENAME_RE.search(body)
...
def discover(memory_root):
    ...
    for char in sorted(os.listdir(oc_root)):
        ...
        # 代号唤醒：从基础包正文抽取「英雄代号」补为别名（供 find 用代号解析，
        # 不写回 .md，仅运行时解析用；不改用户源/记忆包）。
        codename = _extract_codename(getattr(base_pkg, "body", "") or "")
        if codename and codename.lower() not in {a.lower() for a in base_aliases}:
            base_aliases.append(codename)
        ocs.append({
            "name": char,
...
# ---------------------------------------------------------------------------
def _match_keys(oc):
    """返回该 OC 所有“叫名字”可命中的键（小写）。

    含：完整别名 / name / base_title / base_id，以及从别名中派生的
    「部分名键」（首个分隔段，如『示例角色』→『示例角色』），
    用于用户只叫部分名（『示例角色你在干嘛』）时也能唤醒。
    """
    keys = set()
    raw = []
    for a in oc.get("base_aliases", []):
        if a:
            raw.append(a)
    raw.append(oc["name"])
    raw.append(oc["base_title"])
    raw.append(oc["base_id"])
    for r in raw:
        r = (r or "").lower().strip()
        if not r:
            continue
        keys.add(r)
        # 派生部分名键：取首个分隔段（·/•/空格/（/(/：/: 之前），长度≥2
        for sep in ("·", "•", " ", "（", "(", "：", ":"):
            if sep in r:
                seg = r.split(sep, 1)[0].strip()
                if len(seg) >= 2:
                    keys.add(seg)
                break
    return keys
...
def build_parser():
    ...
    f.add_argument("text", help="用户原话，例如『示例角色你在干嘛』")
