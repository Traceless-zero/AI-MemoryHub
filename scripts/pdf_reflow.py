...
方案（全程确定性，无 LLM）：
  ...
  ⑥ --write 落库：保留 front-matter（linked/tags/created 不动），
     只换 body；锚点用 derive_anchors(max_level=4) 重新派生

用法：
  python scripts/pdf_reflow.py <pdf> --out /tmp/reflow.md            # 试跑出稿
  python scripts/pdf_reflow.py <pdf> --write <memory_root> <包相对路径> <事件id>
"""
import os
import re
import sys
...
def write_back(memory_root, pkg_rel, event_id, body):
    ...
    anchors = derive_anchors(body, max_level=4)   # 论文体：最小单元锚点
...
def main():
    ...
    if "--write" in args:
        ...
        print(f"[write] 已落库 {event_id}（锚点 {n} 个，max_level=4）")
    elif "--out" in args:
