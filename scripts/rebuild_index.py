...
确定性全量重建（零 AI、零 token）：
  ...
  2) 重生成 memory/目录结构树.md （复用 hma.tree.build_tree）
...
def _auto_derive(mem, progress=None):
    ...
    c = mem._conn()
    rows = c.execute(
        "SELECT id, filepath, package_id FROM events").fetchall()
    n = 0
    for _rid, filepath, pid in rows:
        if not filepath or not os.path.exists(filepath):
            continue
        with open(filepath, "r", encoding="utf-8") as f:
            pkg = EventPackage.from_markdown(f.read(), filepath)
...
def main(argv, progress=None):
    ...
    repo = _exe_dir()
    # 源码模式（python scripts/rebuild_index.py）下，把仓库根加入 path，
    ...
    try:
        ...
        try:
            ...
            m.close()
        if progress:
            progress("tree", "重生成 目录结构树.md …")
        tree.build_tree(mem)
        summary = "已重建 %d 条事件索引，目录结构树已刷新" % cnt
        return True, summary
    except Exception as e:
