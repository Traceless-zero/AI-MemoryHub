...
TESTS:
  A1 query_anchors(allow_abstain=True) 合法域内查询 -> 不拒答且命中 demo-origin
  ...
  C1 query_features 属性特征归一：黄蓝色的宝石 -> 示例信物
...
# A1 合法域内查询不拒答 + 命中
r = m.query_anchors("黄蓝色的宝石", top_k=1, allow_abstain=True, keywords=["宝石"])
...
# demo-origin 仍合法（示例事件 锚点）。核心锁点 = 域内不拒答（over-abstain 修复）。
GEM_PKGS = ("shengbaoluzhihuo", "cangqiong-zhilei", "demo-origin")
check("A1 域内不拒答+命中宝石实体",
      (not abstain) and pkg is not None and pkg.startswith(GEM_PKGS),
      f"abstain={abstain} top1={pkg}")

# A2 越域拒答
r2 = m.query_anchors("今天天气怎么样", top_k=1, allow_abstain=True, keywords=["今天天气怎么样"])
check("A2 越域拒答",
...
check("C1 属性特征归一->示例信物",
      locked is not None and "示例信物" in locked,
      f"locked={locked}")
