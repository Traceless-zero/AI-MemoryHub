...
锁定行为（与已验证沙箱一致）：
  ...
  (5) check_kw5 仍正确强制确定性叙事维（用整洁包验证；真实 demo-char anchor[7] 缺地点 token
...
def main():
    root = _root
    vfp = os.path.join(root, "memory/原创角色/示例角色/demo-origin.md")
    efp = os.path.join(root, "memory/文章/随笔/存在主义随笔.md")
    demo-char = load_pkg_dict(vfp)
    ...
    adv = {
        "event_date": "—",
        "person": [], "location": [],
        "topic": [{"示例信物": ["黄/橙", "蓝", "双色", "宝石", "价值连城"]}],
        "anchors": [{"Chapter": "X", "about": "y", "keywords": ["示例信物"]}],
    }
    ...
    all_ok &= assert_eq("示例信物 归 锚定物品", "锚定物品" in cov, True)
    all_ok &= assert_eq("示例信物 误归 关键事件?", "关键事件" in cov, False)
