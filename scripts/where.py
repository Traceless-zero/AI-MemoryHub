...
自定位原理（零搜索、确定性）：
  本脚本随包分发，永远位于 <repo>/scripts/where.py，
  故 仓库根 = 本脚本所在目录的父级。不扫盘、不猜。

用法：
  python scripts/where.py            # 人读 / AI 读的文本报告
  python scripts/where.py --json     # 机器可读 JSON（供程序消费）
  python scripts/where.py --quiet    # 只打印 memory 根一行（供 shell 变量）
...
if os.path.isdir(os.path.join(_SELF_DIR, "memory")):
    ...
    REPO = os.path.dirname(_SELF_DIR)      # 标准位：scripts/ 的父级
...
def collect():
    ...
    info = {
        ...
        "tree_md": os.path.join(memory, "目录结构树.md"),
...
def main(argv):
    ...
    if not ok["index_db_exists"]:
        print("\n[提示] index.db 不存在——首次使用请先跑一次: 一键更新记忆索引.exe 或 python scripts/rebuild_index.py")
    return 0
