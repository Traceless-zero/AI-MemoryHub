...
> `aimh-always`（常驻分发）与 `aimh-intake`（前门元路由）都引用本表；改路由只改这一处。
>
...
| 用户意图 | 触发信号 | 路由到 |
|---|---|---|
| 存**通用**文本 / 笔记 / 日志 / 想法（含主动存信号） | "记一下""存进记忆" / 浮现持久事实 / 多轮沉淀结论 | **`aimh-ingest`**（通用分支） |
| 存 / 摘**文章 / 资料 / 论文**（归档 + 理解综述） | "归档这篇""把论文收进记忆""读懂这篇" / 长文资料 | **`aimh-ingest`**（文章/资料分支） |
...
| **召回失败诊断**（该命中没命中） | "召回失败""搜不到""为什么查不到""漏召回" / regress 挂了找原因 | **`aimh-recall-diagnose`**（三层：FM → index.db → 代码流程） |
| **学怎么用 HMA（看操作手册）** | "用户操作手册""怎么用HMA""教我怎么用" | 引擎取 `用户操作手册` 包（见 `aimh-always` 自唤起钩子） |
| 按名查 OC 档案（**不扮演**） | 提到某 OC 名字、想看它档案 | `python scripts/core/oc_registry.py find` + `python -m hma.engine query` |
| **更新路径**（HMA 目录搬了 / 换了盘） | "更新路径""路径变了""HMA 搬家了" | 跑 `python <新位置>/scripts/core/where.py` 刷新 `~/.hma_home` |
...
mcp__aimh__memory_read_section <id> "<章节>"   # L3 取整章正文（MCP 工具，非 CLI）
# daylog 时间轴：读取经 hma.daylog 模块（read_day / days_in_range），无独立 CLI；写入经 scripts/core/daylog_append.py
python scripts/core/oc_registry.py find "<原话>"              # OC 名字 → 基础包
...
- **无状态确定性检索**：检索是确定性查询（关键词 / 锚点 / 时间过滤），绝不按热度 / 新鲜度加权。时间过滤的固定时间词（上个月 / 前天 / 3月份 / X之前 / X以来…）**原样传 `time_hint`**，引擎确定性换算日期，AI 不做日历计算。
- **AI 只理解，脚本确定性写**：理解 / 拆分 / 关联判断归 AI，落库 / 建索引 / 装卸交给 `hma.engine` CLI 与子技能。
- **内容即数据**：拆包触发条件是体积 / 凝聚性，不是作者写法。
- **OC 三层拆包铁律**（经 `oc-dossier`）：① 基础包必有且仅 1 个 = 姓名+形象+语气；② origin 包（背景故事包，默认必有，哪怕一行字也独立成包，仅无背景故事者省略）；③ 拓展包含则分、无则无。
- **项目 README 式拆包**（`aimh-project`）：需求清单 / 项目结构 / 开发日志 / 约定 各成单独 `.md`；项目结构文件禁塞铁律，约定文件收"死规矩"。
