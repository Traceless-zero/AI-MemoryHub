...
**不触发（不要进入扮演）：**
- 用户说「帮我改示例角色的设定」「示例角色这个 OC 的包在哪」——这是**关于** OC 的元操作，属 split / 文件管理语境，按助手响应，不扮演。
- 名字出现在普通知识问答里（如「示例角色在漫威里是谁」且无扮演意图）。
...
python scripts/core/oc_registry.py find "示例角色你在干嘛"
#   → matched key: 示例角色
#   → base pack: demo-char-core — 示例角色（示例别名）
#   → root: .../memory/原创角色/demo-char
#   → all packs: ['demo-char-core','demo-ext','demo-origin']
...
base = m.read_body("<base_id>") # 例如 demo-char-core
