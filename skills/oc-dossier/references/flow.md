...
```
oc-dossier（store 写侧：把 OC 拆成 基础包/故事包/拓展包，落 memory/原创角色/<char>/）
        │
        ▼
memory/原创角色/<char>/events/*.md   （.md 权威源，front-matter 含 aliases/tags）
        │
        ▼
scripts/oc_registry.py  find "用户原话"   （读侧：名字 → 基础包 确定性解析）
        │
        ▼
oc-dossier（wake 分支：加载基础包 → 进入角色 → 惰性召回 → 退出）
```

split 保证「包可被确定性检索」；invoke 保证「检索被确定性使用」。两者靠同一份 `.md` 与 `aliases` 衔接，不引入新状态。
...
- 用户消息里出现 `memory/原创角色/` 下某角色的 `aliases` / `name` / 基础包 `title`，且句子是疑问句、祈使句、或明显对其说话。
...
python scripts/oc_registry.py find "维罗妮卡你在干嘛"
#   → matched key: 维罗妮卡
...
- 把 `base_aliases` + `name` + `base_title` + `base_id` 全部小写，与用户原话（小写）做子串匹配；取**最长命中**者，避免「维」这类短词误命中。
...
- `m.query_anchors(q)` → 在故事包锚点层召回，返回 `[(pkg_id, anchor_title, summary, locator, score)]`。
  - 问「你以前怎么离开回旋镖的」→ `query_anchors("回旋镖 假死")` 先给摘要，必要时 `read_section("<story_id>", locator)` 读整段。
...
假设 `memory/原创角色/suye/` 已按 split 铁律落库：`suye-base`（基础包，aliases=["苏野"]）、`suye-tundra`（故事包）、`suye-ability` 等拓展包。

用户：「苏野，你今天在苔原上听见了什么？」

1. `python scripts/oc_registry.py find "苏野，你今天在苔原上听见了什么？"` → 命中 `苏野` → `base: suye-base`，`root: .../原创角色/suye`。
2. `Memory(root).read_body("suye-base")` → 取得姓名、形象（瘦高、晒斑、考察 vest）、语气（轻声、爱跑题到植物、温和幽默）。
3. 以苏野口吻回应（第一人称、轻声、带点植物比喻），先不翻故事包。
4. 用户追问：「你那次北境失联到底怎么回事？」→ `query_anchors("北境失联")` 取摘要，`read_section("suye-tundra", "北境失联")` 读整段，再据此回应。
